...
class evaluate:
    ...
    def test(self, Lvec,Rvec):
        ...
        def cal(results):
            hits1,hits5,hits10,mrr,mr = 0,0,0,0,0
            for x in results[:,1]:
                if x < 1:
                    hits1 += 1
                if x < 5:
                ...
                mrr += 1/(x + 1)
                mr += (x + 1)
            return hits1, hits5, hits10, mrr, mr
        hits1,hits5,hits10,mrr, mr = cal(results)
        print("Hits@1: ",hits1/len(Lvec)," ","Hits@5: ",hits5/len(Lvec)," ","Hits@10: ",hits10/len(Lvec)," ","MR: ",mr/len(Lvec), " ","MRR: ",mrr/len(Lvec))
        return results